...
1. **escena ↔ motor.** [escena.js](escena.js) es sólo números: paleta,
   ...
   **¿Y CUANDO LO QUE CAMBIA ES LA ANATOMÍA ENTERA?** Entonces la escena
   no lleva los números: lleva el NOMBRE. La silueta del rape vive en
   `FORMAS`, en [bichos/rape-cuerpo.js](bichos/rape-cuerpo.js), con sus
   perfiles, su ojo, sus púas y sus dientes; la escena sólo dice
   `forma: 'tinta'`. Se resuelve UNA VEZ al nacer (`f.F`) y de ahí abajo
   ninguna pieza del dibujo vuelve a preguntar qué rape es. Así se
